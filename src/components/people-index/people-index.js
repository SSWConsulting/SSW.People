...
const PeopleIndex = ({ data, current }) => {
  const allPeople = useMemo(() => buildPeople(data, current), [data]);
      ...
      <div className="mx-2 md:mx-6 flex flex-col lg:flex-row">
        <div className="lg:w-1/4">
          <div className="mx-auto flex flex-col sm:flex-row lg:flex-col lg:w-5/6">
            ...
            {current &&
              process.env.EVENTS_API &&
              process.env.EVENTS_API.length > 4 && (
...
function buildPeople(data, current) {
    ...
    .map(node => {
      ...
      if (dataCRM || isFixedTile) {
        ...
        return {
          ...
          sanitisedName: (current ? '' : 'alumni/') + node.parent.name,
          jobTitle: jobTitle,
          role: !isFixedTile
            ? dataCRM.role
              ? dataCRM.role
          ...
          sanitisedNickname: !isFixedTile
            ? (current ? '' : 'alumni/') + dataCRM.nickname.replace(/\s+/g, '-')
            : node.parent.name,
        };
      }
    })
...
const PeopleIndexWithQuery = props => (
  <Location>
    {({ location }) => (
      <PeopleIndex
        {...props}
...
export default PeopleIndexWithQuery;
